...
var randomString = util.randomString = function(len) {
  len = parseInt(len);
  if (isNaN(len) || len <= 0) {
    len = Math.floor(Math.random() * 20) + 1;
  }
...
var mkdirSync = util.mkdirSync = function(dir) {
  dir && !fs.existsSync(dir) && mkdirp(dir);
};

var mkdirSyncForFile = util.mkdirSyncForFile = function(file) {
  file && mkdirSync(path.dirname(file));
};

util.osTempDir = process.env.TMPDIR || process.env.TMP || process.env.TEMP ||
  ( process.platform === 'win32' ? 'c:\\windows\\temp' : '/tmp');

util.noMilliseconds = function(date){
  date = util.isDate(date) ? date : new Date();
